...
export const legalContent: Record<AppLocale, { privacy: LegalDocument; terms: LegalDocument; labels: { privacy: string; terms: string; company: string } }> = {
  "zh-HK": {
    ...
    privacy: {
      ...
      sections: [
        { title: "我們持有的資料", paragraphs: ["我們可能持有你的姓名、電郵地址、登入及驗證紀錄、語言與顯示設定；你提交的問題、卦象事實、已儲存閱讀、筆記及對話；贊助金額、付款狀態及 Stripe 識別碼；聯絡表格內容；以及防濫用、錯誤與操作記錄。完整付款卡資料由 Stripe 處理，我們不會儲存。"] },
        { title: "使用目的", paragraphs: ["我們使用資料以建立及保護帳戶、提供可重現的閱讀、保存你明確選擇的內容、處理贊助、回覆支援查詢、執行用量與安全限制、偵測濫用、維護服務，並履行適用的法律義務。我們不會出售個人資料。"] },
        { title: "AI 由你選擇", paragraphs: ["DeepSeek 解讀是自願功能。每次閱讀首次使用 AI 前，我們會列明並請你同意傳送卦象事實、經審核的來源及原問題；同一次閱讀的後續對話會沿用這份同意及相同資料範圍。高風險問題會繞過 AI。模型的隱藏推理不會回傳至瀏覽器，也不會儲存在瀏覽器；拒絕使用 AI 不影響確定性結果。"] },
    ...
    terms: {
      ...
      sections: [
        { title: "反思工具，不是專業建議", paragraphs: ["Yi 依固定規則產生卦象事實，並在你同意後，提供用於文化反思的來源材料與 AI 內容。它不是預言，亦不提供醫療、心理健康、法律、金融、投資、安全或其他專業建議。請勿以服務取代合資格專業人士、緊急服務或你自己的判斷。"] },
        { title: "帳戶與可接受使用", paragraphs: ["儲存、筆記、搜尋及對話需要帳戶。你須提供準確資料、妥善保護登入方式，並對帳戶活動負責；不得濫用服務、規避限制、干擾系統、侵犯他人權利、提交違法內容，或未經許可大量擷取來源目錄。"] },
        { title: "隨喜贊助", paragraphs: ["HK$0 也能完成閱讀。HK$1–888 的正數款項，是對數碼服務的隨喜贊助，並非慈善捐款。正數付款由 Stripe Checkout 處理，只有經驗證的 webhook 才會確認付款及顯示結果。退款可向 contact@rich-tide.com 提出，並會按適用法律、付款狀態及具體情況處理。"] },
        ...
        { title: "你的內容與服務材料", paragraphs: ["你保留對問題、筆記及對話內容所擁有的權利，並授予我們一項有限許可，僅限用於本服務的營運、保安、支援、匯出與刪除。應用程式、設計、品牌、程式及來源目錄受各自權利保護；除法律允許外，不得複製或重新分發。"] },
  ...
  "zh-CN": {
    ...
    privacy: {
      ...
      sections: [
        ...
        { title: "使用目的", paragraphs: ["我们使用数据以创建及保护账户、提供可重现阅读、保存你明确选择的内容、处理随喜赞助、回复咨询与支持请求、执行用量与安全限制、检测滥用、维护服务，并履行适用的法律义务。我们不会出售个人数据。"] },
        { title: "AI 由你选择", paragraphs: ["DeepSeek 解读是自愿功能。每次阅读首次使用 AI 前，我们会列明并请你同意发送卦象事实、经审核的来源及原问题；同一次阅读的后续对话会沿用这份同意及相同数据范围。高风险问题会绕过 AI。模型的隐藏推理不会回传至浏览器，也不会存储在浏览器；拒绝使用 AI 不影响确定性结果。"] },
    ...
    terms: {
      ...
      sections: [
        { title: "反思工具，不是专业建议", paragraphs: ["Yi 按固定规则生成卦象事实，并可在你同意后，提供用于文化反思的来源材料与 AI 内容。它不是预言，也不提供医疗、心理健康、法律、金融、投资、安全或其他专业建议。请勿以服务取代合格专业人士、紧急服务或你自己的判断。"] },
        { title: "账户与可接受使用", paragraphs: ["保存、笔记、搜索及对话需要账户。你须提供准确数据、妥善保护登录方式，并对账户活动负责；不得滥用服务、规避限制、干扰系统、侵犯他人权利、提交违法内容，或未经许可批量提取来源目录。"] },
        { title: "随喜赞助", paragraphs: ["HK$0 也能完成阅读。HK$1–888 的正数款项，是对数字服务的随喜赞助，并非慈善捐款。正数付款由 Stripe Checkout 处理，只有经验证的 webhook 才会确认付款及显示结果。退款可向 contact@rich-tide.com 提出，并会按适用法律、付款状态及具体情况处理。"] },
        ...
        { title: "你的内容与服务材料", paragraphs: ["你保留对问题、笔记及对话内容所拥有的权利，并授予我们有限的许可，仅限用于本服务的运营、安全、支持、导出与删除。应用程序、设计、品牌、程序及来源目录受各自权利保护；除法律允许外，不得复制或重新分发。"] },
  ...
  en: {
    ...
    privacy: {
      ...
      sections: [
        ...
        { title: "AI is your choice", paragraphs: ["DeepSeek interpretation is optional. Before a reading first uses AI, we list the cast facts, reviewed sources and original question and ask you to agree to sending them. Follow-up chat for that reading reuses the same consent and data scope. High-stakes questions bypass AI. Hidden model reasoning is not returned to or stored in the browser. Declining AI does not affect the deterministic result."] },
