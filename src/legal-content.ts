...
export const legalContent: Record<AppLocale, { privacy: LegalDocument; terms: LegalDocument; labels: { privacy: string; terms: string; company: string } }> = {
  "zh-HK": {
    ...
    privacy: {
      ...
      intro: "RICHTIDE LIMITED（「我們」）營運 Yi · 易。本政策說明我們收集哪些個人資料、使用原因、分享對象、保存時間，以及你可以如何行使選擇和權利。",
      sections: [
        ...
        { title: "使用目的", paragraphs: ["我們使用資料來建立及保護帳戶、提供可重現的閱讀、保存你明確選擇的內容、處理掛金、回覆支援查詢、落實用量及安全限制、偵測濫用、維護服務，以及履行適用法律責任。我們不出售個人資料。"] },
        { title: "AI 由你選擇", paragraphs: ["DeepSeek 反思是自願功能。每次使用前，你分別選擇是否傳送卦象事實、已審核來源及原問題。高風險問題會繞過 AI。模型的隱藏推理不會回傳至瀏覽器，也不會儲存在瀏覽器。拒絕 AI 不影響確定性結果。"] },
        ...
        { title: "保存與分享", paragraphs: ["未儲存的訪客閱讀會在七日內刪除。匿名分享連結在七日後失效，且不含身份、原問題、筆記或對話。已儲存閱讀、筆記及對話保留至你逐項刪除或關閉帳戶。操作錯誤記錄一般在 90 日後刪除。付款及必要審計資料可按法律、會計、爭議及防詐需要保存較長時間。"] },
        { title: "保安、Cookie 與你的權利", paragraphs: ["我們使用同站 HTTP-only 工作階段、存取控制、加密傳輸及資料最小化。必要 Cookie 用於登入及安全；語言、主題及字體選擇可儲存在你的裝置。你可在設定中匯出或刪除帳戶，亦可要求查閱或更正個人資料。為保障資料，我們可能核實申請人身份。"] },
  ...
  "zh-CN": {
    ...
    privacy: {
      ...
      sections: [
        { title: "我们持有的数据", paragraphs: ["我们可能持有你的姓名、邮箱地址、登录及验证记录、语言与显示设置；你提交的问题、卦象事实、已保存阅读、笔记及对话；随喜赞助金额、付款状态及 Stripe 标识符；联系表单内容；以及防滥用、错误及操作记录。完整支付卡数据由 Stripe 处理，我们不存储。"] },
        { title: "使用目的", paragraphs: ["我们使用数据来创建和保护账户、提供可重现阅读、保存你明确选择的内容、处理随喜赞助、回复支持请求、落实用量与安全限制、检测滥用、维护服务，以及履行适用法律责任。我们不出售个人数据。"] },
        { title: "AI 由你选择", paragraphs: ["DeepSeek 反思是自愿功能。每次使用前，你分别选择是否发送卦象事实、已审核来源及原问题。高风险问题会绕过 AI。模型的隐藏推理不会回传至浏览器，也不会存储在浏览器。拒绝 AI 不影响确定性结果。"] },
        ...
        { title: "保存与分享", paragraphs: ["未保存的访客阅读会在七天内删除。匿名分享链接在七天后失效，且不含身份、原问题、笔记或对话。已保存阅读、笔记及对话保留至你逐项删除或关闭账户。操作错误记录通常在 90 天后删除。付款及必要审计数据可按法律、会计、争议及防欺诈需要保存更长时间。"] },
        { title: "安全、Cookie 与你的权利", paragraphs: ["我们使用同站 HTTP-only 会话、访问控制、加密传输及数据最小化。必要 Cookie 用于登录及安全；语言、主题及字体选择可存储在你的设备。你可在设置中导出或删除账户，也可要求访问或更正个人数据。为保护数据，我们可能核实申请人身份。"] },
    ...
    terms: {
      ...
      sections: [
        ...
        { title: "随喜赞助", paragraphs: ["HK$0 可完成阅读。HK$1-888 的正数款项是对数字服务的随喜赞助，不是慈善捐款。正数付款由 Stripe Checkout 处理，只有经验证的 webhook 才确认付款及显示结果。退款可向 contact@rich-tide.com 提出，并会按适用法律、付款状态及具体情况处理。"] },
