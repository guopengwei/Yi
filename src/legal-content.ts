  ...
  "zh-HK": {
    ...
    privacy: {
      ...
      sections: [
        ...
        { title: "AI 由你選擇", paragraphs: ["DeepSeek 解讀是自願功能。每次使用前，你都可以分別選擇是否傳送卦象事實、經審核的來源及原問題；高風險問題會繞過 AI。模型的隱藏推理不會回傳至瀏覽器，也不會儲存在瀏覽器；拒絕使用 AI 不影響確定性結果。"] },
  ...
  "zh-CN": {
    ...
    privacy: {
      ...
      sections: [
        ...
        { title: "AI 由你选择", paragraphs: ["DeepSeek 解读是自愿功能。每次使用前，你都可以分别选择是否发送卦象事实、经审核的来源及原问题；高风险问题会绕过 AI。模型的隐藏推理不会回传至浏览器，也不会存储在浏览器；拒绝使用 AI 不影响确定性结果。"] },
  ...
  en: {
    ...
    privacy: {
      ...
      sections: [
        ...
        { title: "AI is your choice", paragraphs: ["DeepSeek interpretation is optional. Before each use, you separately choose whether to send cast facts, reviewed sources and your original question. High-stakes questions bypass AI. Hidden model reasoning is not returned to or stored in the browser. Declining AI does not affect the deterministic result."] },
